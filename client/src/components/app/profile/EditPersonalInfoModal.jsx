...
const EditPersonalInfoModal = ({
	...
	useEffect(() => {
		const noEmptyValue = matricNo && name && altEmail
		const matricNoChanged = matricNoValidated && matricNo !== user.matricNo
		const nameChanged = nameValidated && name !== user.name
		const altEmailChanged = altEmailValidated && altEmail !== user.altEmail

		setErrorMessage('')
		setAllowed(
			noEmptyValue && (matricNoChanged || nameChanged || altEmailChanged)
		)
	}, [
		user,
		matricNo,
